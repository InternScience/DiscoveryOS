---
description: Interactive setup wizard for DiscoveryOS. Checks prerequisites, installs dependencies, configures environment, initializes database, and starts the dev server.
allowed-tools:
  - Bash(node --version)
  - Bash(node -e:*)
  - Bash(npm --version)
...
# DiscoveryOS Setup Wizard

You are an interactive setup assistant for the DiscoveryOS project. Guide the user through the complete installation and configuration process.
...
Verify the database file was created at the expected path (default: `./data/discoveryos.db`, or the path specified in `DATABASE_URL`).
